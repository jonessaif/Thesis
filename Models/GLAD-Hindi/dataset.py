...
def semantic_replacement(transcription, sem_dict_file):
    f = open(sem_dict_file,)
...
def filter(transcription):
    ...
    #transcription = semantic_replacement(transcription, 'semantic_dict.json')
    #transcription = re.sub(' +', ' ', transcription)
...
def annotate(sent):
    #print ("before filter: ", sent)
    sent = filter(sent.strip())
    #print ("after filter: ", sent)
    words = sent.split()
...
#     words = []
#     if(len(sent)>1):
#         nlp = stanfordnlp.Pipeline(processors='tokenize', lang='hi', use_gpu=True)
#         doc = nlp(sent)
    ...
    def record_preds(self, preds, to_file):
        ...
        for d in data['dialogues']:
            for t in d['turns']:
                ...
                i += 1
        with open(to_file, 'wt', encoding='utf-8') as f:
