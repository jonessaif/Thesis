...
def run(args):
    ...
    if not args.test:
        ...
        model.run_train(dataset['train_hin'], dataset['validate_hin'], args)
    if args.resume:
    ...
    dev_out = model.run_eval(dataset['validate_hin'], args)
    pprint(dev_out)
...
def get_args():
    ...
    parser.add_argument('--epoch', help='max epoch to run for', default=400, type=int)
    parser.add_argument('--demb', help='word embedding size', default=300, type=int)
    parser.add_argument('--dhid', help='hidden state size', default=150, type=int)
