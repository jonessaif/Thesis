...
def define_DNN_model(utterance_representations_full, num_filters=300, vector_dimension=300, longest_utterance_length=40):

    def process_uttr(turn_uttr):
        unigram = tf.zeros([300], tf.float32)
        bigram = tf.zeros([600], tf.float32)
        trigram = tf.zeros([900], tf.float32)
        for i in range(longest_utterance_length):
            if i<longest_utterance_length-1:
                bigram = tf.math.add(bigram,tf.concat([turn_uttr[i],turn_uttr[i+1]], axis=0))
            if i<longest_utterance_length-2:
                trigram = tf.math.add(trigram,tf.concat([turn_uttr[i],turn_uttr[i+1],turn_uttr[i+2]], axis=0))
            unigram = tf.math.add(unigram,turn_uttr[i])
        print (unigram.shape, bigram.shape, trigram.shape)
        #return tf.concat([unigram, bigram, trigram], axis=0)
        return [unigram, bigram, trigram]
    
    uni, bi, tri = tf.map_fn(process_uttr, utterance_representations_full, dtype=[tf.float32, tf.float32, tf.float32], parallel_iterations=512)
    print (uni.shape, bi.shape, tri.shape)
 
    W1 = tf.Variable(tf.truncated_normal([vector_dimension, vector_dimension], stddev=0.1), name="W1")
    b1 = tf.Variable(tf.constant(0.1, shape=[vector_dimension]), name="b1")
    unigram_representation = tf.nn.sigmoid(tf.matmul(uni, W1) + b1)

    #W2 = tf.Variable(tf.random_normal([2*vector_dimension, vector_dimension]))
    #b2 = tf.Variable(tf.zeros([vector_dimension]))
    W2 = tf.Variable(tf.truncated_normal([2*vector_dimension, vector_dimension], stddev=0.1), name="W2")
    b2 = tf.Variable(tf.constant(0.1, shape=[vector_dimension]), name="b2")
    bigram_representation = tf.nn.sigmoid(tf.matmul(bi, W2) + b2)
    
    W3 = tf.Variable(tf.truncated_normal([3*vector_dimension, vector_dimension], stddev=0.1), name="W3")
    b3 = tf.Variable(tf.constant(0.1, shape=[vector_dimension]), name="b3")
    trigram_representation = tf.nn.sigmoid(tf.matmul(tri, W3) + b3)
    print (unigram_representation.shape, bigram_representation.shape, trigram_representation.shape)

    #hidden_representation = tf.zeros([num_filters], tf.float32)
    hidden_representation = tf.math.add_n([unigram_representation, bigram_representation, trigram_representation])
...
def model_definition(vector_dimension, label_count, slot_vectors, value_vectors, model_type, use_delex_features=False, use_softmax=True, value_specific_decoder=False, learn_belief_state_update=True):
    """
...
#    utterance_representations_delex = tf.placeholder(tf.float32, [None, label_size, 40, vector_dimension])
    ...
    if model_type == "CNN":
        ...
        h_utterance_representation = define_DNN_model(utterance_representations_full, num_filters, vector_dimension, longest_utterance_length)
    ...
    if value_specific_decoder and True:
        if model_type == "CNN":
            ...
            h_utterance_representation_for_full_softmax = define_DNN_model(utterance_representations_full, num_filters, vector_dimension, longest_utterance_length)
