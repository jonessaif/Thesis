...
def xavier_vector(word, D=300):
    ...
    norm1 = norm(rsample)
    rsample_normed = rsample/norm1
    print("-- Generating word vector for:", word, ":::", numpy.sum(rsample_normed))
...
def semantic_replacement(transcription, sem_dict):
    for key, values in sem_dict.items():        
        for v in values:
            if v in transcription:
                transcription = transcription.replace(v, key)
                break          
    return transcription


def process_turn_hyp(transcription, language, sem_dict):
    """
    ...
    transcription = re.sub(' +', ' ', transcription) ## remove multiple spaces in a string
    ...
    #transcription = semantic_replacement(transcription, sem_dict)
    #transcription = re.sub(' +', ' ', transcription)
    if language == "en" or language == "english" or language == "hin" or language == "hindi":# or language == "en" or language == "english":
        transcription = transcription.replace("'", "")

    return transcription


def process_woz_dialogue(woz_dialogue,language,override_en_ontology, sem_dict):
    """
    Returns a list of (tuple, belief_state) for each turn in the dialogue.
    """
    # initial belief state
    # belief state to be given at each turn
    if language == "english" or language == "en" or language == "hindi" or language == "hin" or override_en_ontology:
        null_bs = {}
        null_bs["food"] = "none"
    ...
    for idx, turn in enumerate(woz_dialogue):
        ...
        current_transcription = process_turn_hyp(current_transcription, language, sem_dict)

        #Bug as for some asr its not getting a list of list as it should get
        read_asr = turn["asr"]
        current_asr = []
        for (hyp, score) in read_asr:
            current_hyp = process_turn_hyp(hyp, language, sem_dict)
...
def load_woz_data(file_path, language, sem_dict, percentage=1.0,override_en_ontology=False):
    """
    ...
    for idx in range(0, dialogue_count):
        ...
        current_dialogue = process_woz_dialogue(woz_json[idx]["dialogue"], language, override_en_ontology, sem_dict)
...
def track_woz_data(dialogues, model_variables, word_vectors, dialogue_ontology, sessions, sem_dict):
    """
    ...
    for idx in range(0, dialogue_count):
        ...
        evaluated_dialogue, belief_states = track_dialogue_woz(model_variables, word_vectors, dialogue_ontology, dialogues[idx], sessions, sem_dict)
        evaluated_dialogues.append(evaluated_dialogue)
        list_of_belief_states.append(belief_states)
    print (f"@#@#@#@#@#@ Evaluation Time = {time.time() - s_time} #@#@#@#@#@#@#@#@#@#@#@#@#@#@#@#")
    dialogue_count = len(evaluated_dialogues)
    indexed_dialogues = []
...
def track_dialogue_woz(model_variables, word_vectors, dialogue_ontology, woz_dialogue, sessions, sem_dict):
    """
    ...
    for idx, trans_and_req_and_label_and_currlabel in enumerate(woz_dialogue):
        ...
        for slot in slots_to_track:
            ...
            if slot == "request":
                ...
                updated_belief_state = sliding_window_over_utterance(sessions[slot], example, word_vectors, dialogue_ontology, mx, slot, sem_dict, print_mode=False)
...
def generate_data(utterances, word_vectors, dialogue_ontology, target_slot, sem_dict):
    """
    ...
    feature_vectors = extract_feature_vectors(utterances, word_vectors, sem_dict)
...
def load_word_vectors(file_destination, primary_language="english"):
    ...
    f = codecs.open(file_destination, 'r', encoding="utf-8")
...
def extract_feature_vectors(utterances, word_vectors, sem_dict, ngram_size=3, longest_utterance_length=40, use_asr=False, use_transcription_in_training=False):
    """
    ...
    for idx, utterance in enumerate(utterances):
        ...
        #need to check this method as transcription is missing#
        ...
        for (c_example, asr_coeff) in full_asr[0:asr_count]:
            ...
            if c_example != "":
                # print c_example
                words_utterance = process_turn_hyp(c_example, "hin", sem_dict)
...
def train_run(target_language, override_en_ontology, percentage, model_type, dataset_name, word_vectors, exp_name, sem_dict, dialogue_ontology, model_variables, target_slot, language="hin", max_epoch=20, batches_per_epoch=4096, batch_size=256):
    """
    ...
    _, utterances_train2 = load_woz_data("data/" + dataset_name + "/" + dataset_name + "_train_" + language + ".json", language, sem_dict) # parameter determines which ones are loaded

    utterance_count = len(utterances_train2)

    _, utterances_val2 = load_woz_data("data/" + dataset_name + "/" + dataset_name + "_validate_" + language + ".json", language, sem_dict)
    ...
    feature_vectors, positive_examples, negative_examples = generate_data(utterances_train, word_vectors, dialogue_ontology, target_slot, sem_dict)
    ...
    fv_validation, positive_examples_validation, negative_examples_validation = \
                generate_data(utterances_val, word_vectors, dialogue_ontology, target_slot, sem_dict)
    ...
    for slot in dialogue_ontology:
        if slot not in ratio:
            ratio[slot] = int(batch_size / 2) # fewer negatives
...
def sliding_window_over_utterance(sess, utterance, word_vectors, dialogue_ontology, model_variables, target_slot, sem_dict, print_mode=True):
    """
    """
    ...
    list_of_outputs = test_utterance(sess, utterance, word_vectors, dialogue_ontology, model_variables, target_slot, sem_dict, print_mode)
    belief_state = list_of_outputs[0]

    return belief_state


def test_utterance(sess, utterances, word_vectors, dialogue_ontology, model_variables, target_slot, sem_dict, do_print=True):
    """
    ...
    fv_tuples = extract_feature_vectors(utterances, word_vectors, sem_dict, use_asr=True)
    utterance_count = len(utterances)
    ...
    for idx_hyp, extracted_fv in enumerate(fv_tuples):
        ...
        #print (utterances)
        if target_slot != "request":
            #print (idx_hyp, prev_belief_state)
            #exit()
            prev_value = prev_belief_state[target_slot]
...
class NeuralBeliefTracker:
    ...
    def __init__(self, config_filepath):
        ...
        if not os.path.isfile(word_vector_destination):
            word_vector_destination = "word2vec/vectors_w2v.txt"
        try:
            os.makedirs("models/")
            os.makedirs("results/")
        ...
        sem_dict = config.get("data", "sem_dict")
        ...
        for slot_name in slots:
            #updating this to add english slot names random word vector
            if slot_name not in word_vectors.keys() and " " not in slot_name:
                word_vectors[slot_name] = xavier_vector(slot_name)
            ...
            for value in dialogue_ontology[slot_name]:
                ...
                if " " not in value and value not in word_vectors:
                    ...
                    print("-- Generating word vector for:", value, ":::", numpy.sum(word_vectors[value]))

        # add up multi-word word values to get their representation:
        word_vectors["price"] = xavier_vector("price")
        word_vectors["range"] = xavier_vector("range")
        ...
        for slot in dialogue_ontology:
            ...
            if slot == "request":
                ...
                self.model_variables[slot] = model_definition(word_vector_size, len(dialogue_ontology[slot]), slot_vectors, value_vectors, model_type,\
                ...
                self.model_variables[slot] = model_definition(word_vector_size, len(dialogue_ontology[slot]), slot_vectors, value_vectors, model_type, use_delex_features=self.use_delex_features, \
        ...
        f = open(sem_dict)        
        self.sem_dict = json.loads(f.read())
    ...
    def track_utterance(self, current_utterance, req_slot="", conf_slot="", conf_value="", past_belief_state=None):
        ...
        for slot in self.dialogue_ontology:
            ...
            belief_state = sliding_window_over_utterance(sess, utterance, self.word_vectors, self.dialogue_ontology, self.model_variables, slot, sem_dict)
    ...
    def train(self):
        ...
        for slot in self.dialogue_ontology.keys():
            ...
            stime = time.time()
            train_run(target_language=self.language, override_en_ontology=False, percentage=1.0, model_type=self.model_type, dataset_name=self.dataset_name, \
                    word_vectors=self.word_vectors, exp_name=self.exp_name, sem_dict=self.sem_dict, dialogue_ontology=self.dialogue_ontology, model_variables=self.model_variables[slot], target_slot=slot, language=self.language_suffix, \
    ...
    def test_woz(self):
        ...
        woz_dialogues, training_turns = load_woz_data("data/" + self.dataset_name + "/" + self.dataset_name + "_test_" + self.language_suffix + ".json", self.language, self.sem_dict, override_en_ontology=False)
        ...
        for model_id in range(0, self.num_models):
            ...
            if self.language == "english" or self.language == "en" or self.language == "hindi" or self.language == "hin" or override_en_ontology:
            ...
            for load_slot in slots_to_load:
                ...
                sessions[load_slot] = tf.Session(config=tf_config)
                saver.restore(sessions[load_slot], path_to_load)
            s_time = time.time()
            evaluated_dialogues, belief_states = track_woz_data(woz_dialogues, self.model_variables, self.word_vectors, self.dialogue_ontology, sessions, self.sem_dict)
            list_of_belief_states.append(belief_states) # only useful for interpolating.
        print (f"Total testing time {time.time() - s_time}")
        results = evaluate_woz(evaluated_dialogues, self.dialogue_ontology)

        json.dump(evaluated_dialogues, open("results/woz_tracking.json", "w",encoding="utf-8"),ensure_ascii=False, indent=4)
